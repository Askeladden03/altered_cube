...
hero = {
    "axiom": ["sierra", "subash", "treyst"],
    "bravos": ["basira", "kojo"],
    "lyra": ["fen", "auraq"],
    "muna": ["teija", "rin", "arjun"],
    "ordis": ["waru", "sigismar", "gulrang"],
...
def fill_card_list_from_faction(faction, file):
    card_list = []
    with open(faction + '/' + file) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=':')
        line_count = 0
        for row in csv_reader:
            card_name = f'{row[0]}_{row[1]}'
...
def generate_hero(faction, cards):
    images = [Image.open("images/" + faction + "/" + x + ".jpg") for x in cards]
    ...
    new_im.save("images/hero_" + faction + ".jpg")
...
def generate_pdf_from_faction_jpg():
    ...
    for faction in factions:
        image = Image.open(f'images/all_set/all_{faction}.jpg')
